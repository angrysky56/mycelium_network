#!/usr/bin/env python3
"""
Hybrid mycelium-neural network example.

This script combines the unique features of mycelium networks 
(chemical signaling, resource allocation, adaptive growth)
with traditional neural network learning techniques.
"""
...
class HybridMyceliumNetwork(AdvancedMyceliumNetwork):
    ...
    def _forward_with_dropout(self, features: List[float]) -> List[float]:
        ...
        for hidden_id in self.regular_nodes:
            ...
            hidden_node.activation = 1 / (1 + math.exp(-weighted_sum))
        ...
        for output_id in self.output_nodes:
            ...
            # Apply activation function
            output_node.activation = 1 / (1 + math.exp(-weighted_sum))
    ...
    def _backward_pass(self, 
        ...
        output_node = self.nodes[output_id]
        output_delta = error * output_node.activation * (1 - output_node.activation)
        ...
        for hidden_id in self.regular_nodes:
            ...
            # Calculate delta
            hidden_deltas[hidden_id] = error_term * hidden_node.activation * (1 - hidden_node.activation)
...
def load_wine_data() -> Tuple[List[List[float]], List[int]]:
    """
    ...
    """
    dataset_path = os.path.join(os.path.dirname(__file__), "datasets/wine.csv")
    ...
    with open(dataset_path, 'r') as f:
        ...
        for row in reader:
            ...
            # Extract features (skip the class column)
            feature_values = [float(x) for x in row[1:]]
            features.append(feature_values)
    
    # Convert to binary classification (class 0 vs. others)
    binary_labels = [1 if label == 0 else 0 for label in labels]
    
    print(f"Loaded Wine dataset: {len(features)} samples, {len(features[0])} features")
    print(f"Class distribution: {binary_labels.count(1)} class 0, {binary_labels.count(0)} other classes")
...
def main():
    ...
    print("Hybrid Mycelium Network Example")
    print("==============================")
    ...
    print("\nAvailable datasets:")
    print("1. Breast Cancer (binary classification)")
    print("2. Wine (multi-dimensional)")
    ...
    if dataset_choice == 1:
        ...
        features, labels = load_wine_data()
